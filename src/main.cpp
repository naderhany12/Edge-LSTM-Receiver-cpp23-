...
int main(int argc, char* argv[]) {
    std::cout << "============================================" << std::endl;
    std::cout << "  Initializing C++23 Neural Receiver Engine " << std::endl;
    std::cout << "============================================" << std::endl;
    ...
    if (weights_buf.empty()) {
        std::cerr << "[ERROR] Failed to load weights from file: " << weights_path << std::endl;
        return -1;
    }
    ...
    std::span<const float> b_dense(weights_ptr, dense_b_size);

    // Create LSTM Cell & Buffers
    ...
    std::cout << "\n[INFO] Running Single-Frame Inference over " << sequence_len << " time_steps..." << std::endl;

    // Pure Inference Latency Measurement
    ...
    std::cout << "\n[PERFORMANCE] Pure Inference Latency: " << duration << " us (" 
              << static_cast<float>(duration) / 1000.0f << " ms)" << std::endl;
    ...
    std::cout << "\n[SUCCESS] Inference Engine Operational!" << std::endl;
    return 0;
}